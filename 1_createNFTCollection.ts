...
import { COLLECTION_NAME, COLLECTION_SYMBOL, COLLECTION_DESCRIPTION, FEE_PERCENT, EXTERNAL_URL, METADATA_COLLECTION_URL, IMAGE_URL } from './config';
...
const run = async () => {
  try {
    ...
    const explorerCluster = process.env.NODE_ENV !== 'production' ? '?cluster=devnet' : '';
    ...
    const mintFileName = `./data/collectionMint${process.env.NODE_ENV === 'production' ? 'Mainnet' : 'Devnet'}.txt`;
